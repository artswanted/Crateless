...
namespace GHelper.UI.Nebula.Pages
{
    ...
    public sealed class DisplayPage : NebulaPage
    {
        ...
        public override void Paint(NebulaCanvas c)
        {
            ...
            {
                var lf = c.F(12, FontStyle.Bold);
                string gl = NebulaText.T("Colour presets: GameVisual  ↗", "Цветовые пресеты: GameVisual  ↗");
                c.Txt(gl, 256, row, lf, c.IsHover("display:gvpage") ? th.Text : th.Accent);
                c.Hit(c.R(256, row - 18, c.TextWidth(gl, lf) + 8, 26), "display:gvpage");
                row += 40;
            }
        ...
        public override bool Click(string id, Point at, NebulaForm form)
        {
            switch (id)
            {
                ...
                case "display:windows":
                    ...
                    return true;
                case "display:gvpage": form.ShowPage("visual"); return true;
                case "display:visual":
                {
